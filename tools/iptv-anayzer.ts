...
function extractUrlsAndMacs(text: string): UrlToMacMap {
    // Expressions régulières pour les URL et les adresses MAC
    const urlRegex = /http:\/\/[^\s]+?\/c\/?/g;
    const macRegex = /([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}/g;

    // Trouver toutes les URL avec leurs indices
    ...
    for (let i = 0; i < urlsWithIndices.length; i++) {
        ...
        // Déterminer la zone de recherche pour les MACs
        const endIndex =
        ...
        // Extraire les MACs dans cette zone (conversion explicite en `string[]`)
        ...
        // Ajouter les MACs à l'URL dans la map
